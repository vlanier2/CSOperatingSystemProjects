...
static volatile int ticks, proc_count = 0, ticks_since_rfsh = 0;
volatile bool sigusr1_rcvd = false;
int quantum_ms, ms_past = 0, pid[MAXPROGRAMS];
...
static void sigalrm_handler(UNUSED int sig) {
    ...
    ++ticks_since_rfsh;
    if (ticks_since_rfsh >= REFRESH_EVERY) {
        print_proc_info(pid, ticks, ms_past, current_process, proc_count);
        ticks_since_rfsh = 0;
    }
